(function ($) {
  ...
  $(window).on('load', function () {
    ...
    // Replace Euro symbols and labels with Lei site-wide (non-destructive)
    try {
      ...
      nodes.forEach(function (n) {
        ...
        n.nodeValue = n.nodeValue.replace(/\u20AC|€|EUR/g, function () { return 'Lei'; });
        n.nodeValue = n.nodeValue.replace(/Lei\s*\u20AC|Lei\s*€/g, 'Lei');
      });
    } catch (e) { /* noop */ }
  });
  /*-----------------
        Menu Stick - оптимизирован с requestAnimationFrame для уменьшения работы в основном потоке
    -----------------*/
  ...
  function updateHeader() {
    ...
    // В начале страницы (scroll < 200px) - header всегда виден
    if (scroll < 200) {
      ...
      // При скролле вниз - скрываем header
      if (scrollDirection === 'down') {
        header.removeClass('stick');
        header.addClass('header-hidden');
      } 
      // При скролле вверх - показываем header
  ...
  /*------ ScrollUp - отложенная инициализация для уменьшения работы в основном потоке -------- */
  ...
  if (document.readyState === 'loading') {
    window.addEventListener('load', function() {
      if ('requestIdleCallback' in window) {
        requestIdleCallback(initScrollUp, { timeout: 1000 });
      } else {
        setTimeout(initScrollUp, 500);
      }
    });
  } else {
    if ('requestIdleCallback' in window) {
  ...
  //sidebar sticky - отложенная инициализация для избежания принудительной компоновки
  ...
  /*------ Wow Active ---- - отложенная инициализация для избежания принудительной компоновки */
  if ('requestAnimationFrame' in window) {
    requestAnimationFrame(function() {
      new WOW().init();
    });
  ...
  /*---------------------
        Select active - отложенная инициализация для избежания принудительной компоновки
    --------------------- */
  ...
  // Isotope active - отложенная инициализация для избежания принудительной компоновки
  if ($('.grid').length) {
    if ('requestAnimationFrame' in window) {
      requestAnimationFrame(function() {
  ...
  /* --- SwiperJS - отложенная инициализация для уменьшения работы в основном потоке --- */
    ...
    function checkAndInit() {
      if (typeof Swiper !== 'undefined') {
        if ('requestIdleCallback' in window) {
          ...
          setTimeout(initSwiperGroups, 100);
        }
      } else {
        // Swiper еще не загружен, ждем еще
  ...
  function initSwiperGroups() {
    // Дополнительная проверка перед инициализацией
    if (typeof Swiper === 'undefined') {
      console.warn('Swiper не загружен, пропускаем инициализацию');
      return;
    }
  ...
  // Инициализируем Swiper после загрузки всех скриптов (включая Swiper)
  // Ждем события load, чтобы убедиться, что все скрипты загружены
  if (document.readyState === 'loading') {
    window.addEventListener('load', function() {
      // Даем время на загрузку динамических скриптов
      setTimeout(initSwipers, 500);
    });
  } else {
    // Страница уже загружена, но Swiper может загружаться динамически
  ...
  // Video popup - отложенная инициализация для уменьшения работы в основном потоке
  function initMagnificPopup() {
    if ($('.popup-youtube').length && typeof $.fn.magnificPopup === 'function') {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() {
      if ('requestIdleCallback' in window) {
        requestIdleCallback(initMagnificPopup, { timeout: 1000 });
      } else {
  ...
  // Оптимизированный resize handler с throttling для уменьшения работы в основном потоке
  var resizeTimeout;
  ...
  // Вызываем resize один раз при загрузке
  if ('requestAnimationFrame' in window) {
    requestAnimationFrame(function() {
  ...
  // Masonry - отложенная инициализация для уменьшения работы в основном потоке
  var msnry = null;
  function initMasonry() {
    if ($('#masonry').length > 0 && typeof Masonry !== 'undefined') {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() {
      if ('requestIdleCallback' in window) {
        requestIdleCallback(initMasonry, { timeout: 1000 });
      } else {
  ...
  // Оптимизация: используем requestAnimationFrame для батчинга чтения offset
  if ('requestAnimationFrame' in window) {
    requestAnimationFrame(function() {
...
function switchBilled() {
  ...
  // Оптимизация: батчим все DOM операции для избежания принудительной компоновки
  if ('requestAnimationFrame' in window) {
    requestAnimationFrame(function() {
      if (checkBox.is(':checked')) {
        // Батчим все операции для checked
        forYear.addClass('display-year');
        billYear.addClass('active');
        billMonth.removeClass('active');
        forMonth.removeClass('display-month');
      } else {
        // Батчим все операции для unchecked
        forYear.removeClass('display-year');
        billMonth.addClass('active');
        billYear.removeClass('active');
        forMonth.addClass('display-month');
      }
    });
  } else {
...
//Perfect Scrollbar - отложенная инициализация для избежания принудительной компоновки
var ps, ps2;
function initPerfectScrollbar() {
  if (document.querySelector('.mobile-header-wrapper-inner')) {
    ps = new PerfectScrollbar('.mobile-header-wrapper-inner');
  }
  if (document.querySelector('.sidebar-canvas-container')) {
    ps2 = new PerfectScrollbar('.sidebar-canvas-container');
  }
}
// Отложенная инициализация после полной загрузки страницы
if (document.readyState === 'loading') {
  window.addEventListener('load', function() {
    if ('requestAnimationFrame' in window) {
      requestAnimationFrame(function() {
        requestAnimationFrame(initPerfectScrollbar);
      });
    } else {
      setTimeout(initPerfectScrollbar, 100);
    }
  });
} else {
  if ('requestAnimationFrame' in window) {
...
function odometerCounter() {
  if ($('.odometer').length > 0) {
    $('.odometer').appear(function (e) {
      var $this = $(this);
      // Проверяем, не инициализирован ли уже этот odometer
      if (!$this.data('odometer-initialized')) {
        var countNumber = $this.attr('data-count');
        $this.html(countNumber);
        // Помечаем как инициализированный, чтобы не обновлять повторно
