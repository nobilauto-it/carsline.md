...
(function ($) {
  ...
  $.fn.carouselTicker = function (options) {
    ...
    var _init = function () {
      ...
      // initialize current position cache (to avoid forced reflow)
      ticker.currentPosition = 0;
    ...
    var _moveTicker = function () {
      ...
      // Update cached position instead of reading from DOM (avoids forced reflow)
      ticker.currentPosition += ticker.directionSwitcher * ticker.settings.speed;
      
      // step ticker moving - single DOM write
      ticker.$list.css(mode, ticker.currentPosition + "px");
      
      // depending of direction change offset list for effect Infinity rotate
      if (ticker.settings.direction === "prev") {
        if (Math.abs(ticker.currentPosition) >= itemsSize) {
          ticker.currentPosition = 0;
      ...
      if (ticker.settings.direction === "next") {
        if (ticker.currentPosition >= 0) {
          ticker.currentPosition = -itemsSize;
    ...
    function _calcItemsWidth() {
      ...
      ticker.itemsWidth = 0;
      // Batch all DOM reads before writes to avoid forced reflow
      var itemData = [];
      ...
      // Now read all dimensions at once (single reflow)
      for (var i = 0; i < itemData.length; i++) {
        var item = itemData[i];
        var style = item.currentStyle || window.getComputedStyle(item);
        var margin = parseFloat(style.marginLeft) + parseFloat(style.marginRight);
        ticker.itemsWidth += item.getBoundingClientRect().width + margin;
      }
    }
    ...
    function _eventDragAndDrop() {
      ...
      ticker.$list.on(ticker.eventTypes.mousedown, function (e) {
        var start = e.clientX || event.touches[0].pageX,
          ...
          $this = $(this),
          // Use cached position instead of reading from DOM (avoids forced reflow)
          posList = ticker.currentPosition,
          posListY = ticker.currentPosition;
          ...
          if (flag) {
            if (ticker.settings.mode === "horizontal") {
              ...
              if (posList - diff >= 0 && ticker.directionSwitcher === 1) {
                ticker.currentPosition = -ticker.itemsWidth;
                $this.css("left", ticker.currentPosition + "px");
                posList = ticker.currentPosition;
              ...
              // if drag more right side
                ...
                ticker.currentPosition = posList - diff;
                $this.css("left", ticker.currentPosition + "px");
              }
            } else if (ticker.settings.mode === "vertical") {
              // if drag more top side
              if (posListY - diffY >= 0 && ticker.directionSwitcher === 1) {
                ticker.currentPosition = -ticker.itemsHeight;
                $this.css("top", ticker.currentPosition + "px");
                posListY = ticker.currentPosition;
                startY = e.clientY || event.touches[0].pageY;
                diffY = 0;
              }
              // if drag more bottom side
              else if (
                ...
                ticker.currentPosition = posListY - diffY;
                $this.css("top", ticker.currentPosition + "px");
              }
            }
          }
